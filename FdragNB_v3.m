function F = FdragNB_v3(t,x, Elsys, current, wave)
...
eKz = exp(wave.k * attachNodePos(3,:));
omegat_phase = -wave.omegaCal*t+wave.phaseCal;
sinSignalsAt0 = sin(omegat_phase);
cosSignalsAt0 = cos(omegat_phase);

Kxy =[wave.kXCal,wave.kYCal]*attachNodePos(1:2,:);

sinKxy = sin(Kxy);
cosKxy = cos(Kxy);


Uwave_XY = omegaZa*((sinSignalsAt0.*cosKxy + cosSignalsAt0.*sinKxy).*eKz);
Uwave_Z  = omegaZa*((-sinSignalsAt0.*sinKxy + cosSignalsAt0.*cosKxy).*eKz);
